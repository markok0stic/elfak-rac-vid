...
board = cv.aruco.GridBoard(BOARD_SIZE, MARKER_SIZE, MARKER_SEPARATION, aruco_dict)
...
def calibrate(files : str):
    ...
    _, mtx, dist, _, _ = cv.aruco.calibrateCameraAruco(corners_list,  id_list, counter, board, img_gray.shape, None, None)
...
def video(cap, camera_matrix, dist_coeffs, new_camera_matrix, roi):
    while True:
        ...
        corners, ids, rejectedImgPoints = cv.aruco.detectMarkers(frame_remapped_gray, aruco_dict, parameters=arucoParams)
        cv.aruco.refineDetectedMarkers(frame_remapped_gray, board, corners, ids, rejectedImgPoints)

        im_with_aruco_board = frame

        if type(ids) is np.ndarray and ids.any() != None:
            im_with_aruco_board = cv.aruco.drawDetectedMarkers(frame, corners, ids, (0,255,0))
            rvec = ()
            tvec = ()
            retval, rvec, tvec = cv.aruco.estimatePoseBoard(corners, ids, board, camera_matrix, dist_coeffs, rvec, tvec)
            if retval != 0:
